...
<div class="admincp-header">
    ...
    <div class="admincp-header-r">
        <div class="manager">
            <dl>
                <dt class="name">{{ session('admin_name') }}</dt>
                <dd class="group">{{ $group_info[session('group_id')] }}</dd>
            </dl>
            <span class="avatar">
                <input name="file" type="file" class="admin-avatar-file" id="_pic" title="设置管理员头像"/>
                <img alt="" nctype="admin_avatar" src="{{ session('admin_img') ? session('admin_img') : '_images/login/admin.png'  }}">
            </span>
        </div>
        <ul class="operate nc-row">
            <li><a class="homepage show-option" target="_blank" href="{{ url('/') }}" title="新窗口打开网站">&nbsp;</a></li>
            <li><a class="login-out show-option" href="{{ url('syslogin/logout') }}" title="安全退出管理中心">&nbsp;</a></li>
        </ul>
    </div>
    <div class="clear"></div>
</div>
