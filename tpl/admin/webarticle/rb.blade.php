...
<div class="layui-tab layui-tab-brief">
    ...
    <div class="layui-tab-content">
        ...
        <form action="" method="post" class="ajax-form">
            选中项：
            <button type="button" class="layui-btn  layui-btn-small ajax-action" data-action="{{ url('restore') }}">全部恢复</button>
            <button type="button" class="layui-btn layui-btn-danger layui-btn-small ajax-action" data-action="{{ url('delete') }}">永久删除</button>
                ...
                <table class="layui-table">
                    ...
                    @foreach($lists as $v)
                        <tr>
                            ...
                            <td>{{ $v['delete_time'] }}</td>
                            <td>
                                <a href="{{ url('restore?id='.$v['id']) }}" class="layui-btn layui-btn-xs ajax-post">还原</a>
                                <a href="{{ url('edit?id='.$v['id']) }}" class="layui-btn layui-btn-normal layui-btn-xs">编辑</a>
                                <a href="{{ url('delete?id='.$v['id'].'&force=1') }}" class="layui-btn layui-btn-danger layui-btn-xs ajax-delete">永久删除</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
